...
const generateModel = async (options) => {
  ...
  const { modelName, tableName, attributes, omitId, primary, index, foreign, unique, comboUnique, softDelete, timestamps, force } = options;
  ...
  if (fs.existsSync(modelPath) && !force) {
    ...
    const indexes = `indexes: [${[sparsedIndices, sparsedUnique, simpleComboUnique, sparsedComboUnique].join("\n")}],`;
    ...
    await prettier.format(modelTemplate, {
      parser: "babel"
    }).then((formattedCode) => {
      fs.writeFileSync(modelPath, formattedCode);

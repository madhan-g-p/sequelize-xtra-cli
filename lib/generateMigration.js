...
const generateMigraion = async ({ modelPath, fileName }) => {
  ...
  if (!options.modelPath && !options.fileName) {
    errored('You must provide either model path or file name');
    return;
  }
  if (options.modelPath && options.fileName) {
    errored('cannot provide both model path and file name');
    return;
  }
...
`;
  ...
  await prettier.format(migrationContent, {
    parser: "babel"
  }).then(async(formattedCode) => {
    await fs.writeFileSync(migrationPath, formattedCode);
